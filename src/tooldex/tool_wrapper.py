...
def _default_utility_factory() -> str:
    env_cmd = os.environ.get("UTILITY_CMD")
    if env_cmd:
        return env_cmd

    if shutil.which("tooldex"):
        return "tooldex codex"

    python_exe = shlex.quote(sys.executable or "python")
    return f"{python_exe} -m tooldex codex"
