...
def _default_codex_config_path() -> Path:
    return Path(__file__).resolve().parents[1] / "configs" / "codex.toml"
...
def bundled_codex_config_path() -> Path:
    """Return the path to the bundled Codex configuration template."""

    return _default_codex_config_path()


def default_user_codex_config_path() -> Path:
    """Return the default user-scoped Codex configuration path."""

    return Path.home() / ".tooldex" / "codex.toml"
