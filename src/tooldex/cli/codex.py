...
from tooldex.core.config import (
    CodexConfig,
    ConfigError,
    bundled_codex_config_path,
    default_user_codex_config_path,
    load_codex_config,
...
def init_codex_config(path_override: str | None, force: bool) -> Path:
    """Copy the bundled Codex template into a user-accessible location."""

    destination = Path(path_override).expanduser() if path_override else default_user_codex_config_path()
    template = bundled_codex_config_path()

    if destination.exists() and not force:
        raise ConfigError(f"Codex config '{destination}' already exists; pass --force to overwrite.")
